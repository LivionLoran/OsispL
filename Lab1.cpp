...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_PAINT: {
		...
		/* get device's context */
		hDC = BeginPaint(hwnd, &PaintStruct);
		/*create a context compatible with the window context in memory */
		hCompatibleDC = CreateCompatibleDC(hDC);
		/* make the loaded bitmap current in a compatible context */
		hOldBitmap = SelectObject(hCompatibleDC, hBitmap);
		StretchBlt(hDC, leftPos, topPos, width, height, hCompatibleDC, 0, 0, width, height, SRCAND);
		/* make the old bitmap current again*/
		SelectObject(hCompatibleDC, hOldBitmap);
		/* remove compatible context */
		DeleteDC(hCompatibleDC);
		/* release the main context, completing the redrawing of the window stage */
